...
const GOOGLE_SEARCH_API_KEY = 'AIzaSyDbzjXOy4-gHl6uCq96q76udf3ehn6nV8k'; // <--  API Key de Google
const GOOGLE_SEARCH_ENGINE_ID = 'e76448d8b670b498e'; // <--  ID de motor de búsqueda
const YOUTUBE_SEARCH_URL = 'https://www.googleapis.com/youtube/v3/search';
const GOOGLE_SEARCH_URL = 'https://www.googleapis.com/customsearch/v1';

async function obtenerResumenGoogle(consulta) {
    const url = `${GOOGLE_SEARCH_URL}?key=${GOOGLE_SEARCH_API_KEY}&cx=${GOOGLE_SEARCH_ENGINE_ID}&q=${encodeURIComponent(consulta)}`;
    try {
        const respuesta = await fetch(url);
        const datos = await respuesta.json();
        
        if (datos.items && datos.items.length > 0) {
            const primerResultado = datos.items[0];
            return `${primerResultado.snippet} <br><br> Fuente: <a href="${primerResultado.link}" target="_blank">${primerResultado.displayLink}</a>`;
        } else {
            return "No encontré información sobre eso.";
        }
    } catch (error) {
        console.error('Error al obtener la información de Google:', error);
        return "Hubo un error al obtener la información de Google. Verifica tu API Key y el ID del motor de búsqueda.";
    }
}
...
async function procesarMensaje(mensaje) {
    if (detectarIntencion(mensaje)) {
        ...
        let consultaGoogle = mensaje
            ...
            .replace("¿Qué es ", "")
            .replace("")
        return await obtenerResumenGoogle(consultaGoogle); // <-- Llamada a la nueva función
    }
}
...
//<script async src="https://cse.google.com/cse.js?cx=e76448d8b670b498e">
//</script>
